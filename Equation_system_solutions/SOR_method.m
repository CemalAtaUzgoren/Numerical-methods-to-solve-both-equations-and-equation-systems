function [root_matrix] = SOR_method(matrix,incon,epsilon_s,lamda)
%This function solves the given agumented matrix(n,n+1) by using the  gauss
%seidel method and returns a vector that contains the roots of the system of
%equations
...
while a==1
    ...
    for i=1:1:size(matrix,1)%row
        ...
        incon_reminder(i)=incon(i);
        matrix(i,size(matrix,2))=matrix(i,size(matrix,2))/matrix(i,c);%root candidate
        matrix(i,size(matrix,2))=(matrix(i,size(matrix,2))*lamda)+((1-lamda)*incon_reminder(i));
        incon(i)=matrix(i,size(matrix,2)); %updating the incon
